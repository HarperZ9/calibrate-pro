...
def build_correction_lut(levels, trc_r, trc_g, trc_b, M_display, black_xyz, white_Y, size=33):
    ...
    inv_trc_r = interp1d(trc_r, levels, kind="linear", bounds_error=False, fill_value=(0.0, 1.0))
    inv_trc_g = interp1d(trc_g, levels, kind="linear", bounds_error=False, fill_value=(0.0, 1.0))
    inv_trc_b = interp1d(trc_b, levels, kind="linear", bounds_error=False, fill_value=(0.0, 1.0))
...
if __name__ == "__main__":
    ...
    print(f"\n  Baseline: avg dE = {np.mean(valid_baseline):.2f}, max dE = {np.max(valid_baseline):.2f}")
    ...
    lut = build_correction_lut(levels, trc_r, trc_g, trc_b, M_display, black_xyz, white_Y, size=33)
