...
if __name__ == "__main__":
    ...
    def show(r, g, b, s=1.2):
        canvas.config(
            bg=f"#{max(0, min(255, int(r * 255 + 0.5))):02x}{max(0, min(255, int(g * 255 + 0.5))):02x}{max(0, min(255, int(b * 255 + 0.5))):02x}"
        )
        root.update()
        time.sleep(s)
    ...
    try:
        ...
        pass  # DWM LUT removal is optional
    ...
    def ramp(mk):
        ...
        for v in levels:
            r, g, b = mk(v)
            show(r, g, b, 0.8 if v > 0.1 else 1.5)
            xyz = mxyz(0.8 if v > 0.1 else 1.5)
            out.append(xyz if xyz is not None else np.zeros(3))
        return np.array(out)

    wr = ramp(lambda v: (v, v, v))
    rr = ramp(lambda v: (v, 0, 0))
    gr = ramp(lambda v: (0, v, 0))
    br = ramp(lambda v: (0, 0, v))
    blk = wr[0].copy()
    for a in [wr, rr, gr, br]:
        a -= blk
    wr[0] = 0
    rr[0] = 0
    gr[0] = 0
    ...
    # Bradford
    sw = SRGB_TO_XYZ @ np.array([1, 1, 1])
    dwn = dw_xyz / dw_xyz[1]
    swn = sw / sw[1]
    sc_c = BRADFORD_MATRIX @ swn
    dc = BRADFORD_MATRIX @ dwn
    adapt = BRADFORD_INVERSE @ np.diag(dc / sc_c) @ BRADFORD_MATRIX

    def _xy(x):
        s = sum(x)
        return (x[0] / s, x[1] / s) if s > 0 else (0, 0)
    ...
    def correct(r, g, b):
        ...
        if lum < 0.03:
            result = rgb_in * (1 - lum / 0.03) + result * (lum / 0.03)
    ...
    for i, (name, r, g, b) in enumerate(COLORCHECKER):
        ...
        if xyz is not None:
            ...
            if bd >= 0:
                ch = de - bd
                st = "PASS" if de < 2 else "WARN" if de < 3 else "FAIL"
