...
Bring Calibrate Pro back as a full-time flagship product with public presentation on the same level as the Project Telos flagship line, while turning the codebase into an enterprise-grade display calibration platform that can later interlock with Buildc/BuildLang for verified color kernels, typed effects, and receipt-backed AI/ML workflows.

Calibrate Pro is not a replacement for the five Telos flagships. It is a flagship product in the Build line: the measurement-grade display/color surface that proves the same thesis in a physical domain. Buildc/BuildLang remains the Telos substrate for language-level reliability, security, and effect accountability.
...
- A Build/Telos-adjacent flagship whose story is physics-grade measurement: constants, calibration, repeatability, and reality checked against instruments.
...
- [ ] R9: Add a Buildc/BuildLang bridge plan inside the product: shared color-science golden vectors, BuildLang receipt compatibility, capability/effect taxonomy alignment, and future `.build` kernels for color math and report verification.
...
Receipts should be JSON-serializable, stable enough for tests, and designed to map later onto BuildLang's `buildc check --receipt` and policy profile vocabulary.
...
### Track D: Buildc/BuildLang Bridge
...
- Add a BuildLang-facing manifest describing function names, inputs, expected outputs, tolerances, and capability effects.
- Keep Buildc/BuildLang as the future kernel verifier for pure color math and receipt/policy checks.
- Do not claim Calibrate Pro is implemented in BuildLang until a `.build` kernel compiles and is verified.

Initial bridge vocabulary:

| Calibrate Pro Domain | BuildLang Effect/Receipt Mapping |
| --- | --- |
...
| Policy receipt verification | `buildlang-check-policy/v1`-compatible ideas, not identical schema yet |
...
- `README.md` - flagship positioning, install, proof posture, enterprise track summary, Buildc/BuildLang bridge note.
- `USAGE.md` - enterprise commands, receipts, policy workflow, dry-run examples.
- `docs/index.html` - flagship landing page.
- `docs/TECHNICAL.md` - receipt architecture, profile manifests, policy gates, BuildLang bridge.
...
- `tests/test_buildlang_bridge.py` - golden-vector manifest and bridge metadata tests.
...
- [ ] S6: Buildc/BuildLang bridge artifacts exist as stable manifests/golden vectors with honest future-facing status.
...
python -m pytest tests/test_enterprise_receipts.py tests/test_enterprise_policy.py tests/test_enterprise_inventory.py tests/test_buildlang_bridge.py -q
...
Buildc/BuildLang bridge verification, when a `.build` bridge artifact exists:

```powershell
cargo test --manifest-path C:\dev\public\pubscan\buildlang\compiler\Cargo.toml semantic_corpus_manifest --quiet
cargo test --manifest-path C:\dev\public\pubscan\buildlang\compiler\Cargo.toml semantic_corpus_receipt --quiet
```
...
7. **BuildLang bridge corpus**: golden-vector manifest and future `.build` kernel contract.
...
- BuildLang can verify C-backed semantic corpus and effect receipts today, but Calibrate Pro is Python; the bridge must begin with manifests/golden vectors rather than claiming native BuildLang implementation.
